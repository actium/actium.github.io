// https://github.com/SaeX/calendar-yearview-blocks
(function ($) {
    ...
    $.fn.calendar_yearview_blocks = function (options) {
        ...
        var start = function () {
            ...
            var current_date = settings.to_date;
            var start_date = new Date(settings.from_date.getTime());
            var requested_start_date = settings.from_date;
            ...
            for (var i = 0; i <= 52; i++) { // For each week, generate a column
                ...
                for (var j = 0; j < 7; j++) { // For each weekday, generate a row
                    ...
                    if (settings.strict_range && start_date < requested_start_date) {
                        render = false;
                    }
                    ...
                    if (render) {
                        ...
                        item_html += '<rect class="day" width="11" height="11" y="' + y + '" fill="' + color + match_today + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                        if (items.length === 2) { // Fill a trangle for the 2nd
                            ...
                            item_html += '<polygon points="' + 0 + ',' + (y + 11) + ' ' + 0 + ',' + y + ' ' + 11 + ',' + y + '" fill="' + color_1 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                            ...
                            item_html += '<polygon points="' + 0 + ',' + (y + 8) + ' ' + 0 + ',' + y + ' ' + 11 + ',' + y + ' ' + 11 + ',' + (y + 8) + '" fill="' + color_1 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                            item_html += '<polygon points="' + 0 + ',' + (y + 4) + ' ' + 0 + ',' + y + ' ' + 11 + ',' + y + ' ' + 11 + ',' + (y + 4) + '" fill="' + color_2 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                            ...
                            item_html += '<polygon points="' + 0 + ',' + (y + 11) + ' ' + 0 + ',' + (y + 6) + ' ' + 6 + ',' + (y + 6) + ' ' + 6 + ',' + (y + 11) + '" fill="' + color_1 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                            item_html += '<polygon points="' + 0 + ',' + (y + 6) + ' ' + 0 + ',' + y + ' ' + 6 + ',' + y + ' ' + 6 + ',' + (y + 6) + '" fill="' + color_2 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                            item_html += '<polygon points="' + 6 + ',' + (y + 6) + ' ' + 6 + ',' + y + ' ' + 11 + ',' + y + ' ' + 11 + ',' + (y + 6) + '" fill="' + color_3 + '" data-items="' + items.join(", ") + '" data-date="' + settings.prettify_date(data_date) + '"/>';
                        }
                    }
                ...
                if (week53) {
                    $(".weeks52").removeClass("weeks52").addClass("weeks53");
                } else {
                    $(".weeks53").removeClass("weeks53").addClass("weeks52");
                }
            ...
            if (settings.start_monday === true) {
                ...
                loop_html += '<text text-anchor="middle" class="wday" dx="-13" dy="22">{0}</text>'.formatString(settings.day_names[0]) +
                    '<text text-anchor="middle" class="wday" dx="-13" dy="48">{0}</text>'.formatString(settings.day_names[1]) +
                    '<text text-anchor="middle" class="wday" dx="-13" dy="74">{0}</text>'.formatString(settings.day_names[2]);
            }
        ...
        var mouseEnter = function (evt) {
            ...
            var text = settings.format_tip(date, items);
        ...
        var settings = $.extend({
            ...
            to_date: new Date(),
            strict_range: false,
            prettify_date: function (date_string) {
                return date_string;
            },
            format_tip: function (date, items) {
                return "{0}: <br />{1}".formatString(date, items);
            }
        }, options);
